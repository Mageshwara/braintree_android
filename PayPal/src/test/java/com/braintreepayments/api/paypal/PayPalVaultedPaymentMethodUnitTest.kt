...
class PayPalVaultedPaymentMethodUnitTest {

    @Test
    fun fromJson_parsesInstrumentResponse() {
        val json = JSONObject(
            """
            {"data":{"fetchVaultedPaymentMethod":{"payer":null,"paymentMethods":[
              {"label":"CREDIT UNION 1","imageUrl":"https://x/generic_bank.png",
               "lastDigits":"3357","type":"BANK","subtype":null}]}}}
            """.trimIndent()
        )

        val result = PayPalVaultedPaymentMethod.fromJson(json)
    ...
    fun fromJson_parsesDisplayOnlyResponse() {
        val json = JSONObject(
            """
            {"data":{"fetchVaultedPaymentMethod":{"payer":{"email":"buyer@example.com",
    ...
    fun fromJson_returnsEmptyNoFi_whenDataNull() {
        val json = JSONObject("""{"data":{"fetchVaultedPaymentMethod":null}}""")
    ...
    fun exception_fromGraphQLResponse_readsErrorClassAndMessage() {
        val json = JSONObject(
            ...
            {"errors":[{"message":"PayPal access token not found for merchant account.",
             ...
             "data":{"fetchVaultedPaymentMethod":null}}
            """.trimIndent()
        )

        val exception = PayPalVaultedPaymentMethodException.fromGraphQLResponse(json)
