...
class PayPalCheckoutRequestUnitTest {
    ...
    @Test
    @Throws(JSONException::class)
    fun `creates requestBody and sets editBillingAgreementJwt when present`() {
        val request = PayPalCheckoutRequest("1.00", true).apply {
            editBillingAgreementJwt = "edit-billing-agreement-jwt"
        }
        ...
        val jsonObject = JSONObject(requestBody)
        assertEquals(
            "edit-billing-agreement-jwt",
            jsonObject.getString("edit_billing_agreement_jwt")
        )
    }
