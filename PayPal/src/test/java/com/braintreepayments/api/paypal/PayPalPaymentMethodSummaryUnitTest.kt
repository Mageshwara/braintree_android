...
class PayPalPaymentMethodSummaryUnitTest {
    ...
    fun fromJson_parsesInstrumentResponse() {
        val json = JSONObject(
            """
            {"data":{"getSavedPaypalPaymentMethod":{"payer":null,"paymentMethods":[
    ...
    fun fromJson_parsesDisplayOnlyResponse() {
        val json = JSONObject(
            """
            {"data":{"getSavedPaypalPaymentMethod":{"payer":{"email":"buyer@example.com",
    ...
    fun fromJson_returnsEmptyNoFi_whenDataNull() {
        val json = JSONObject("""{"data":{"getSavedPaypalPaymentMethod":null}}""")
    ...
    fun exception_fromGraphQLResponse_readsErrorClassAndMessage() {
        val json = JSONObject(
            ...
            {"errors":[{"message":"PayPal access token not found for merchant account.",
             ...
             "data":{"getSavedPaypalPaymentMethod":null}}
            """.trimIndent()
        )
