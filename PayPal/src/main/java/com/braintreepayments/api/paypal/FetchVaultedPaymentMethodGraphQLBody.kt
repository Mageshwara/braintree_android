...
/**
 * Builds the request body for the shared `fetchVaultedPaymentMethod` GraphQL operation. One query,
 * two callers — only the `fetchPaymentMethodType` discriminator and the identity field differ.
 */
internal object FetchVaultedPaymentMethodGraphQLBody {
    ...
    private const val QUERY =
        "query FetchVaultedPaymentMethod(\$input: FetchVaultedPaymentMethodInput!) { " +
            "fetchVaultedPaymentMethod(input: \$input) { " +
