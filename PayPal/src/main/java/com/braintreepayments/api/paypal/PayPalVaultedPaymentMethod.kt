...
/**
 * A single vaulted funding instrument returned by the `fetchVaultedPaymentMethod` query.
 *
 ...
 */
@ExperimentalBetaApi
data class PayPalVaultedInstrument internal constructor(
    val label: String?,
    val imageUrl: String?,
 ...
 */
@ExperimentalBetaApi
data class PayPalVaultedPayer internal constructor(
    val email: String?,
    val isEditable: Boolean?,
)

/**
 * The parsed result of a `fetchVaultedPaymentMethod` read.
 *
 ...
 */
@ExperimentalBetaApi
data class PayPalVaultedPaymentMethod internal constructor(
    val paymentMethods: List<PayPalVaultedInstrument>,
    val payer: PayPalVaultedPayer?,
) {
     ...
     * No-FI response.
     */
    val primaryInstrument: PayPalVaultedInstrument?
        get() = paymentMethods.firstOrNull()

    internal companion object {

        private const val DATA_KEY = "data"
        private const val FETCH_VAULTED_PAYMENT_METHOD_KEY = "fetchVaultedPaymentMethod"
        ...
        fun fromJson(response: JSONObject): PayPalVaultedPaymentMethod {
            val payload = response
                .optJSONObject(DATA_KEY)
                ?.optJSONObject(FETCH_VAULTED_PAYMENT_METHOD_KEY)
            ...
            return PayPalVaultedPaymentMethod(paymentMethods = instruments, payer = payer)
        }
    }
}
